...
def parse_args():
    parser = argparse.ArgumentParser(description="Run non-censored B2 vs BestCurveLocal on the historical RHC direct-outcome setup.")
    parser.add_argument("--input-csv", type=Path, default=PROJECT_ROOT / "data" / "rhc" / "raw_rhc.csv")
    parser.add_argument("--output-dir", type=Path, default=PROJECT_ROOT / "non_censored" / "outputs" / "rhc_b2_vs_c3")
...
def fit_full_predictions(x: np.ndarray, w: np.ndarray, z: np.ndarray, a: np.ndarray, y: np.ndarray, *, random_state: int) -> tuple[np.ndarray, np.ndarray]:
    x_full = _stack_final_features(x, w, z)

    b2 = BaselineCausalForestDML(
        n_estimators=200,
        min_samples_leaf=20,
        random_state=random_state,
        cv=5,
    )
    b2.fit_baseline(x_full, a, y)
    pred_b2 = b2.effect(x_full).ravel()

    best = BestCurveLocalNCCausalForest(random_state=random_state)
    best.fit_components(x, a, y, z, w)
    pred_best = best.effect_from_components(x, w, z).ravel()
    return pred_b2, pred_best
...
def render_curve_png(curve_df: pd.DataFrame, output_path: Path):
    ...
    ax.set_title("RHC direct outcome: non-censored B2 vs BestCurveLocal ranking curve")
...
def main():
    ...
    t0 = time.time()
    pred_b2, pred_best = fit_full_predictions(x, w, z, a, y, random_state=args.random_state)
    ...
    for model_name, pred in [
        ...
        rows.append(metrics)
    summary_df = pd.DataFrame(rows)[["model", "mean_pred_cate", "std_pred_cate", "median_pred_cate", "pct_positive_pred", "dr_ate_all", "dr_ate_top20", "policy_gain_positive_vs_none", "policy_gain_top20_vs_none", "policy_value_positive", "policy_value_top20"]]
    summary_df.to_csv(output_dir / "rhc_b2_vs_best_curve_summary_long.csv", index=False)
    ...
    curve_df.to_csv(output_dir / "rhc_b2_vs_best_curve_curve.csv", index=False)

    prediction_df = pd.DataFrame({"A": a, "Y": y, "pred_b2": pred_b2, "pred_best_curve": pred_best, "tau_dr": tau_dr})
    prediction_df.to_csv(output_dir / "rhc_b2_vs_best_curve_predictions.csv", index=False)

    subtitle = f"n={metadata['n']}, treatment rate={metadata['treatment_rate']:.1%}, mean outcome={metadata['mean_outcome']:.2f}, total runtime={elapsed:.1f}s"
    summary_png = output_dir / "rhc_b2_vs_best_curve_summary.png"
    curve_png = output_dir / "rhc_b2_vs_best_curve_curve.png"
    render_table_png(summary_df, "RHC non-censored B2 vs BestCurveLocal", subtitle, summary_png)
    render_curve_png(curve_df, curve_png)
    render_pdf([summary_png, curve_png], output_dir / "rhc_b2_vs_best_curve_bundle.pdf")
