...
from grf.non_censored.models import B2SummaryFeatureComboNCCausalForest  # noqa: E402


FEATURE_CANDIDATES = ("h_diff", "abs_h_diff", "q_centered", "agreement")


def _feature_combos():
    combos = [()]
    for size in range(1, len(FEATURE_CANDIDATES) + 1):
        combos.extend(itertools.combinations(FEATURE_CANDIDATES, size))
    return combos


MODEL_SPECS = []
for combo in _feature_combos():
    if not combo:
        label = "B2Sum (ProxyDup)"
    else:
        label = "B2Sum (ProxyDup + " + " + ".join(combo) + ")"
    MODEL_SPECS.append((label, combo))
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run non-censored basic12 ProxyDup-B2Sum feature-combo benchmark."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "non_censored" / "outputs" / "benchmark_basic12_b2sum_proxydup_feature_combo",
    )
...
def _evaluate_model(case, name, combo):
    start = time.time()
    model = B2SummaryFeatureComboNCCausalForest(extra_bridge_features=combo)
...
def main() -> int:
    ...
    for case_spec in selected_cases:
        ...
        rows = []
        for name, combo in MODEL_SPECS:
            rows.append(_evaluate_model(case, name, combo))
