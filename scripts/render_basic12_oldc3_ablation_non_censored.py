...
from grf.non_censored.benchmarks import CASE_SPECS, _make_cfg  # noqa: E402


OUTPUT_DIR = PROJECT_ROOT / "non_censored" / "outputs" / "benchmark_basic12_oldc3_ablation"
DESKTOP = Path("/Users/kyoungeuihong/Desktop")
OUTPUT_CSV = DESKTOP / "csf_grf_new_oldc3_ablation_basic12_non_censored.csv"
OUTPUT_PNG = DESKTOP / "csf_grf_new_oldc3_ablation_basic12_non_censored.png"
OUTPUT_PDF = DESKTOP / "csf_grf_new_oldc3_ablation_basic12_non_censored.pdf"
...
def _render_summary_page(df: pd.DataFrame, *, title: str, subtitle: str, output_png: Path):
    ...
    fig, ax = plt.subplots(figsize=(22, 8.5), dpi=200)
    bg = "#ffffff"
    cell_bg = "#f3f4f6"
    alt_bg = "#ffffff"
    header_bg = "#243042"
    ...
    col_widths = [0.05, 0.25, 0.08, 0.08, 0.07, 0.07, 0.07, 0.08, 0.07, 0.07]
    table = ax.table(
        cellText=rows,
        colLabels=columns,
        colLoc="center",
        cellLoc="center",
        bbox=[0.03, 0.07, 0.94, 0.79],
...
def _prepare_case_df(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    out["time_display"] = out["time_sec"].map(lambda v: f"{float(v):.1f}s")
...
def _render_case_page(df: pd.DataFrame, *, title: str, subtitle: str):
    ...
    fig.text(0.5, 0.965, title, fontsize=20.5, fontweight="bold", color=text, ha="center", va="top")
    fig.text(0.5, 0.925, subtitle, fontsize=11.5, color=muted, ha="center", va="top")
...
def _format_case_title(case_spec: dict) -> str:
    ...
    return f"{base}, n={cfg.n}, p={cfg.p_x}, seed={cfg.seed}, censoring rate=0%"
...
def main() -> int:
    ...
    subtitle = "Aggregated over the original basic 12 non-censored synthetic cases."
    summary_fig = _render_summary_page(
        summary,
        title="Basic 12-Case Non-Censored Benchmark: Old C3 Feature Ablations",
        subtitle=subtitle,
    ...
    with PdfPages(OUTPUT_PDF) as pdf:
        ...
        for case_id in sorted(case_df["case_id"].unique()):
            ...
            fig = _render_case_page(
                ...
                subtitle=f"10-model benchmark | Non-censored | Case {int(case_id):02d}",
            )
