...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Strict apples-to-apples censored Final Model feature-only benchmark. "
            "Keeps current Final defaults fixed and varies only the final-stage "
            "survival summary feature set."
        )
    )
...
def _resolve_output_dir(args: argparse.Namespace) -> Path:
    ...
    return (
        PROJECT_ROOT / "outputs" / "benchmark_censored_final_model_feature_search_strict_fullrefit_v1"
    ).resolve()


def _base_strict_kwargs(args: argparse.Namespace) -> dict[str, object]:
    return {
        "target": args.target,
...
def _variant_specs() -> list[dict[str, object]]:
    return [
        {
            "variant_id": "v00_pair_default",
            "name": "Current Final default (pair)",
            "params": {"surv_scalar_mode": "pair"},
        },
        {
            "variant_id": "v01_full",
            "name": "Feature set: full",
            "params": {"surv_scalar_mode": "full"},
        },
        {
            "variant_id": "v02_qhat_diff_pts",
            "name": "Feature set: qhat diff points",
            "params": {"surv_scalar_mode": "qhat_diff_pts"},
        },
        {
            "variant_id": "v03_qhat_full_pts",
            "name": "Feature set: qhat full points",
            "params": {"surv_scalar_mode": "qhat_full_pts"},
        },
        {
            "variant_id": "v04_qhat_stats",
            "name": "Feature set: qhat stats",
            "params": {"surv_scalar_mode": "qhat_stats"},
        },
        {
            "variant_id": "v05_s_stats",
            "name": "Feature set: survival stats",
            "params": {"surv_scalar_mode": "s_stats"},
        },
        {
            "variant_id": "v06_censor_stats",
            "name": "Feature set: censor stats",
            "params": {"surv_scalar_mode": "censor_stats"},
        },
        {
            "variant_id": "v07_disagreement",
            "name": "Feature set: disagreement",
            "params": {"surv_scalar_mode": "disagreement"},
        },
        {
            "variant_id": "v08_qhat_censor",
            "name": "Feature set: qhat + censor",
            "params": {"surv_scalar_mode": "qhat_censor"},
        },
        {
            "variant_id": "v09_qhat_disagree",
            "name": "Feature set: qhat + disagreement",
            "params": {"surv_scalar_mode": "qhat_disagree"},
        },
        {
            "variant_id": "v10_all_lite",
            "name": "Feature set: all lite",
            "params": {"surv_scalar_mode": "all_lite"},
        },
    ]
...
def main() -> None:
    ...
    case_specs = [
        spec for spec in CASE_SPECS if case_ids is None or int(spec["case_id"]) in case_ids
    ]
    ...
    rows: list[dict[str, object]] = []
    print(
        f"[strict-feature-search] variants={len(variant_specs)} cases={len(case_specs)} workers={args.workers}",
        flush=True,
    )
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            print(
                f"[strict-feature-search] completed case_id={int(case_spec['case_id'])} "
