...
MODEL_ORDER = [
    ...
    "New C3",
    "Augmented-DML (PCI)",
    "Augmented-DML + Stats (PCI)",
    "Augmented-DML + Poly2 q (PCI)",
    "Augmented-DML + Extra h (PCI)",
    "Augmented-DML + Stats + Poly2 q + Extra h (PCI)",
]


def parse_args():
    parser = argparse.ArgumentParser(
        description="Run non-censored basic12 experiments for Augmented-DML improvement ideas."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "non_censored" / "outputs" / "benchmark_basic12_augmented_dml_improvements",
    )
...
def main() -> int:
    ...
    candidate_specs = [
        ...
        ("Augmented-DML + Stats (PCI)", OldC3AugmentedDMLBridgeStatsNCCausalForest),
        ("Augmented-DML + Poly2 q (PCI)", OldC3AugmentedDMLPoly2QNCCausalForest),
