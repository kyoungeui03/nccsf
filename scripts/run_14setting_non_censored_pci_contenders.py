...
MODEL_SPECS = [
    ("PureB2", PureB2NCCausalForest),
    ("B2Sum (PCI)", B2SummaryNCCausalForest),
    ("OldC3Raw (PCI)", OldC3RawNCCausalForest),
    ("AugDML (PCI)", AugDMLNCCausalForest),
    ("AugSP (PCI)", AugSPNCCausalForest),
    ("BroadAugDML (PCI)", BroadAugDMLNCCausalForest),
    ("BroadAugSP (PCI)", BroadAugSPNCCausalForest),
]


def parse_args():
    parser = argparse.ArgumentParser(description="Run non-censored structured 14-setting benchmark for PCI-only contender models.")
    parser.add_argument("--setting-ids", nargs="*")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "non_censored" / "outputs" / "benchmark_structured_14settings_pci_contenders",
    )
