...
try:  # pragma: no cover
    ...
    from .run_5model_benchmark import (  # noqa: E402
        ...
        StrictOracleCensoredSurvivalForest,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.60,
        help="Horizon quantile used by prepare_case.",
    )
    parser.add_argument(
        "--train-seed",
        type=int,
    ...
    parser.add_argument(
        ...
        help=(
            "Models to evaluate. Supported: 'Final Model Oracle', 'Final Model', "
            "'R-CSF Baseline', 'Strict Baseline Oracle', 'Strict Baseline'."
        ),
    )
    parser.add_argument(
        "--model-seed",
...
def _fit_model(
    ...
    *,
    ...
    if name == "Strict Baseline Oracle":
        model = StrictOracleCensoredSurvivalForest(
            cfg=train_case.cfg,
            dgp=train_case.dgp,
            target=target,
            horizon=horizon,
        ...
        t0 = time.time()
        model.fit_oracle(x, a, time_obs, event, z, w, train_case.U)
        return model, time.time() - t0

    if name == "Strict Baseline":
    ...
    for s_idx, setting in enumerate(settings):
        ...
        for case_spec in case_specs:
            ...
            for t_idx, target in enumerate(targets):
                ...
                train_case = prepare_case(train_spec, target=target, horizon_quantile=float(args.horizon_quantile))
                test_case = prepare_case(test_spec, target=target, horizon_quantile=float(args.horizon_quantile))
