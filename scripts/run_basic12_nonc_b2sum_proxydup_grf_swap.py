...
from grf.non_censored.models import B2SummaryProxyDupGRFNCCausalForest  # noqa: E402


def parse_args():
    parser = argparse.ArgumentParser(description="Run basic12 non-censored B2Sum ProxyDup GRF swap.")
...
def main() -> int:
    ...
    for case_spec in case_specs:
        ...
        start = time.time()
        model = B2SummaryProxyDupGRFNCCausalForest()
        model.fit_components(case["X"], case["A"], case["Y"], case["Z"], case["W"])
        preds = model.effect_from_components(case["X"], case["W"], case["Z"]).ravel()
        row = _metric_row("B2Sum Swap NC (GRF + outer residuals)", preds, case["true_cate"], time.time() - start)
