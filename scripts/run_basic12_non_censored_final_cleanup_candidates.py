...
MODEL_ORDER = [
    ...
    "Augmented-DML + Extra h(1200,3) (PCI)",
    "SP-AugFull logit + extra(1200,3) [dup|full_refit]",
    "SP-AugFull logit + extra(1200,3) [dup|cached_oof]",
    "SP-AugFull logit + extra(1200,3) [dup|fold_ensemble]",
    "SP-AugFull logit + extra(1200,3) [broad_dup|cached_oof]",
]


def parse_args():
    parser = argparse.ArgumentParser(
        description="Run non-censored basic12 final cleanup candidates."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "non_censored" / "outputs" / "benchmark_basic12_non_censored_final_cleanup",
    )
...
def _candidate_specs():
    return [
        ...
        ("Augmented-DML + Extra h(1200,3) (PCI)", OldC3AugmentedDMLExtraH1200NCCausalForest),
        (
            "SP-AugFull logit + extra(1200,3) [dup|full_refit]",
        ...
        (
            ...
            lambda: SinglePassAugmentedFullNCCausalForest(
                ...
                h_min_samples_leaf=3,
                nuisance_feature_mode="dup",
                prediction_nuisance_mode="cached_oof",
            ),
        ),
        (
            "SP-AugFull logit + extra(1200,3) [dup|fold_ensemble]",
