...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run B2 vs BestCurveLocal on raw RHC for 30-day RMST.")
    parser.add_argument("--input-csv", type=Path, default=PROJECT_ROOT / "data" / "rhc" / "raw_rhc.csv")
    parser.add_argument("--output-dir", type=Path, default=PROJECT_ROOT / "outputs" / "rhc_b2_vs_c3_30day")
...
def evaluate_r_csf_variant(obs_df: pd.DataFrame, feature_cols: list[str], horizon: float, num_trees: int, target: str) -> tuple[np.ndarray, float]:
    ...
    with tempfile.TemporaryDirectory(dir=PROJECT_ROOT / "outputs", prefix="rhc_r_") as tmp_dir:
        ...
        cmd = ["Rscript", str(R_CSF_SCRIPT), str(input_path), ",".join(feature_cols), str(float(horizon)), str(int(num_trees)), str(output_path), target]
        t0 = time.time()
...
def evaluate_best_curve_variant(X: np.ndarray, W: np.ndarray, Z: np.ndarray, A: np.ndarray, time_obs: np.ndarray, event: np.ndarray) -> tuple[np.ndarray, float]:
    model = BestCurveLocalCensoredPCISurvivalForest(target="RMST", horizon=None, random_state=42)
    t0 = time.time()
...
def render_table_png(df: pd.DataFrame, title: str, subtitle: str, output_path: Path) -> None:
    ...
    table = ax.table(cellText=fmt_df.values, colLabels=fmt_df.columns, loc="center", cellLoc="center", colLoc="center", bbox=[0.02, 0.02, 0.96, 0.78])
...
def render_curve_png(curve_df: pd.DataFrame, target_label: str, output_path: Path) -> None:
    ...
    for name, group in curve_df.groupby("model"):
        ax.plot(group["share_treated_by_policy"] * 100.0, group["dr_ate_in_selected_group"], marker="o", linewidth=2.5, label=name)
    ax.set_xlabel("Top-k policy share treated (%)")
    ax.set_ylabel("DR estimated effect in selected group")
    ax.set_title(f"RHC 30-day {target_label}: B2 vs BestCurveLocal ranking curve")
...
def main() -> int:
    ...
    cohort_df = pd.DataFrame([{"n": metadata["n"], "treatment_rate": metadata["treatment_rate"], "event_rate_30d": metadata["event_rate_30d"], "survival_rate_30d": metadata["survival_rate_30d"], "mean_rmst_30": metadata["mean_rmst_30"], "num_x_features": len(x_cols), "num_w_features": len(w_cols), "num_z_features": len(z_cols)}])
    cohort_png = output_dir / "rhc_cohort_overview.png"
    render_table_png(cohort_df, "RHC 30-day cohort overview", "Raw RHC with treatment A=swang1, time=t3d30, event=dth30", cohort_png)

    bundle_paths = [cohort_png]
    target_summaries = []
    all_curves = []

    for target, horizon, outcome_col, target_label in [("RMST", 30.0, "rmst_30", "RMST at 30 days")]:
        ...
        best_pred, best_time = evaluate_best_curve_variant(X, W, Z, A, time_obs, event)

        model_rows = []
        for model_name, preds, elapsed, backend in [
            ("B2  R-CSF baseline (X+W+Z)", b2_pred, b2_time, "installed R grf"),
            ("BestCurveLocal  PCI curve-summary forest", best_pred, best_time, "econml.grf"),
        ]:
            metrics = policy_metrics(preds, psi1, psi0)
            model_rows.append({"Model": model_name, "Mean Pred CATE": metrics["mean_pred_cate"], "SD Pred CATE": metrics["std_pred_cate"], "Median Pred CATE": metrics["median_pred_cate"], "Pct Positive Pred": metrics["pct_positive_pred"], "DR ATE (all)": metrics["dr_ate_all"], "DR ATE (top20)": metrics["dr_ate_top20"], "Policy Gain vs None (positive)": metrics["policy_gain_positive_vs_none"], "Policy Gain vs None (top20)": metrics["policy_gain_top20_vs_none"], "Time (s)": elapsed, "Backend": backend, "target": target})
        ...
        results_csv = output_dir / f"rhc_b2_vs_best_curve_{target.replace('.', '_')}_results.csv"
        results_df.to_csv(results_csv, index=False)
        table_png = output_dir / f"rhc_b2_vs_best_curve_{target.replace('.', '_')}_table.png"
        render_table_png(results_df.drop(columns=["target"]), f"RHC 30-day {target_label}", "B2 vs BestCurveLocal on raw RHC using doubly robust policy-style evaluation", table_png)
        bundle_paths.append(table_png)

        curve_df = pd.concat([df for df in all_curves if df["target"].iloc[0] == target], ignore_index=True)
        curve_csv = output_dir / f"rhc_b2_vs_best_curve_{target.replace('.', '_')}_curve.csv"
        curve_df.to_csv(curve_csv, index=False)
        curve_png = output_dir / f"rhc_b2_vs_best_curve_{target.replace('.', '_')}_curve.png"
        render_curve_png(curve_df, target_label, curve_png)
        bundle_paths.append(curve_png)

    summary_csv = output_dir / "rhc_b2_vs_best_curve_summary_long.csv"
    pd.concat(target_summaries, ignore_index=True).to_csv(summary_csv, index=False)
    pdf_path = output_dir / "rhc_b2_vs_best_curve_results_bundle.pdf"
