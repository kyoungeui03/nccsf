#!/usr/bin/env python3
"""Run the strict censored baseline on the full 22-setting x 12-case benchmark.

This runner uses the standardized appendix horizon rules:
...
try:  # pragma: no cover
    ...
    from single_file_censored_models.strict_censored_baseline import (  # type: ignore  # noqa: E402
        StrictEconmlXWZCensoredSurvivalForest,
    )


TARGETS = ["RMST", "survival.probability"]
MODEL_NAME = "Strict Conditional"
MODEL_COUNT = 1
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--censoring-estimator",
        choices=["cox", "nelson-aalen", "kaplan-meier", "km"],
        default="cox",
        help="Censoring nuisance estimator for the strict baseline (default: cox for conditional censoring).",
    )
...
def _resolve_output_dir(args: argparse.Namespace) -> Path:
    ...
    return (PROJECT_ROOT / "outputs" / "strict_conditional_q90_h3_full22").resolve()
...
def _make_run_metadata(
    *,
    ...
    return {
        ...
        "cv": int(args.cv),
        "censoring_estimator": str(args.censoring_estimator),
...
def _persist_checkpoint(results: pd.DataFrame, *, output_dir: Path) -> None:
    ...
    if not summary.empty:
        summary.to_csv(output_dir / "strict_conditional_q90_h3_full22_summary.csv", index=False)
    ...
    *,
    ...
    if existing_row is not None:
        ...
        if progress_hook is not None:
            progress_hook(row, resumed=True)
        return row
    ...
    model = StrictEconmlXWZCensoredSurvivalForest(
        target=target,
        horizon=horizon,
    ...
    if save_row_hook is not None:
        save_row_hook(row)
    if progress_hook is not None:
        progress_hook(row, resumed=False)
    return row
            ...
            for setting in settings:
                ...
                for target in targets:
                    for horizon_spec in _target_horizon_specs(target, survival_quantile=float(args.horizon_quantile)):
                        ...
                        existing_row = completed_lookup.get(
                            (
                                case_id,
                                setting_id,
                                target,
                                str(horizon_spec["label"]),
                                MODEL_NAME,
                            )
                        )
                        ...
                        _evaluate_case_target_horizon(
                            ...
                            cv=args.cv,
                            censoring_estimator=args.censoring_estimator,
