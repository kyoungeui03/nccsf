#!/usr/bin/env python3
"""Run the direct R-CSF baseline on the full 22-setting x 12-case benchmark.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--grf-num-trees", type=int, default=200)
    parser.add_argument("--grf-min-node-size", type=int, default=20)
...
def _resolve_output_dir(args: argparse.Namespace) -> Path:
    ...
    return (PROJECT_ROOT / "outputs" / "r_csf_direct_q90_h3_full22").resolve()
...
def _persist_checkpoint(results: pd.DataFrame, *, output_dir: Path) -> None:
    ...
    if not summary.empty:
        summary.to_csv(output_dir / "r_csf_direct_q90_h3_full22_summary.csv", index=False)
