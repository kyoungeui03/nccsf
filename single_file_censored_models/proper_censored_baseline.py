"""Single-file reference implementation of ProperNoPCICensoredSurvivalForest.

This file keeps the current project behavior for the "proper censored baseline"
used in benchmark comparisons:

    - explicit a_res / y_res
    - final learner path implemented through a custom CausalForestDML wrapper
    - no PCI in nuisance fitting (observed_only=True)
    - no broad_dup (nuisance_feature_mode='dup')
    - no clipping
    - final-stage features: [X, W, Z, surv1_pred, surv0_pred, surv_diff_pred]
...
def _fit_censoring_survival(time, event, estimator="nelson-aalen"):
    ...
    raise ValueError(
        "This single-file proper baseline supports only marginal censoring "
...
def _compute_survival_based_yhat(s_hat_1, s_hat_0, w_hat, t_grid, *, target, horizon):
    """Current survival-based mean nuisance used by the proper censored baseline."""

    w_hat = np.asarray(w_hat, dtype=float).ravel()
    if target == "RMST":
        dt = np.diff(np.concatenate([[0.0], np.asarray(t_grid, dtype=float).ravel()]))
        y_hat_1 = np.c_[np.ones(s_hat_1.shape[0]), s_hat_1] @ np.diff(
            np.concatenate([[0.0], np.asarray(t_grid, dtype=float).ravel(), [float(np.max(t_grid))]])
        )
        y_hat_0 = np.c_[np.ones(s_hat_0.shape[0]), s_hat_0] @ np.diff(
            np.concatenate([[0.0], np.asarray(t_grid, dtype=float).ravel(), [float(np.max(t_grid))]])
        )
        del dt
    else:
        horizon_index = np.searchsorted(np.asarray(t_grid, dtype=float).ravel(), float(horizon), side="right")
        if horizon_index == 0:
...
def _build_final_features_raw_surv(x, raw_w, raw_z, bridge):
    """Final features used by ProperNoPCICensoredSurvivalForest."""
...
class _ProperNoPCINuisance:
    """Nuisance layer for the current proper censored baseline path."""
    ...
    def _fit_event_survival_models(self, nuisance_time, nuisance_delta, surv_features, a):
        if self._event_survival_estimator != "cox":
            raise ValueError("This single-file proper baseline supports event_survival_estimator='cox' only.")
        treated_mask = a == 1
        control_mask = a == 0
    ...
    def _compute_m_pred(self, q_pred, h1_pred, h0_pred, s_hat_1, s_hat_0):
        ...
        if self._m_pred_mode != "survival":
            raise ValueError("This single-file proper baseline supports m_pred_mode='survival' only.")
        return _compute_survival_based_yhat(
            s_hat_1,
            s_hat_0,
            q_pred,
            self._t_grid,
    ...
    def train(self, is_selecting, folds, Y, T, X=None, W=None, Z=None, sample_weight=None, groups=None):
        ...
        sc_for_y_tilde = _predict_censoring_survival_at_values(self._censor_model, y_tilde_eval_time)
...
class _ProperBridgeOutputNuisance(_ProperNoPCINuisance):
    def predict(self, Y, T, X=None, W=None, Z=None, sample_weight=None, groups=None):
...
class _ProperFinalModelWrapper:
    ...
    def _transform(self, X, W, Z, nuisances):
        y_res, a_res, surv1_pred, surv0_pred, surv_diff_pred = nuisances
        bridge = {
            "surv1_pred": surv1_pred,
            "surv0_pred": surv0_pred,
        ...
        x_final = _build_final_features_raw_surv(X, w_for_final, z_for_final, bridge)
...
class _ProperNoPCICensoredDML(CausalForestDML):
    """CausalForestDML subclass that reproduces the proper censored baseline path."""
    ...
    def __init__(
        ...
        *,
        ...
        q_poly_degree=2,
        q_clip=0.0,
        y_tilde_clip_quantile=None,
        y_res_clip_percentiles=(0.0, 100.0),
        h_kind="extra",
        h_n_estimators=600,
    ...
    def _gen_ortho_learner_model_nuisance(self):
        return _ProperBridgeOutputNuisance(
    ...
    def _gen_ortho_learner_model_final(self):
        return _ProperFinalModelWrapper(
...
class ProperNoPCICensoredSurvivalForest:
    """Single-file version of the current ProperNoPCICensoredSurvivalForest."""
    ...
    def __init__(
        ...
        *,
        ...
        if prediction_nuisance_mode != "full_refit":
            raise ValueError("This single-file proper baseline supports prediction_nuisance_mode='full_refit' only.")
        if event_survival_estimator != "cox":
            raise ValueError("This single-file proper baseline supports event_survival_estimator='cox' only.")
        if m_pred_mode != "survival":
            raise ValueError("This single-file proper baseline supports m_pred_mode='survival' only.")
    ...
    def fit_components(self, X, A, time, event, Z, W):
        ...
        self._dml_model = _ProperNoPCICensoredDML(
            target=self._target,
            horizon=self._horizon,
...
ProperCensoredBaseline = ProperNoPCICensoredSurvivalForest
