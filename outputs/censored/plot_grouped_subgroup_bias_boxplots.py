...
def _plot_grouped_boxplots(
    ...
    *,
    ...
    models = [m for m in models_order if m in set(plot_df["model"].astype(str).unique())]
    ...
    for s_idx, setting_id in enumerate(settings):
        ...
        for m_idx, model_name in enumerate(models):
            ...
            colors.append(model_to_color.get(model_name, "#999999"))
    ...
    legend_handles = [
        Patch(facecolor=model_to_color.get(m, "#999999"), edgecolor="#333333", alpha=0.55, label=m)
        for m in models
    ]
...
def main() -> int:
    ...
    requested_models = list(args.models) if args.models else list(DEFAULT_MODELS)
    if requested_models:
        model_set = set(requested_models)
